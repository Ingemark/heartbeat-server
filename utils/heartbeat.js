...
function processRequest(req, res, storage) {
  ...
  multi_get.exec(processHeartbeatData(heartbeat_data, storage, req, res));
}
...
function processHeartbeatData(heartbeat_data, storage, req, res) {
  return function (err, replies) {
    ...
    if (needsToCreateNewSession(session_id, sessions, heartbeat_data, new_timestamp)) {
      ...
      var heartbeat_response = createHeartbeatResponse(heartbeat_data,
        session_id, new_timestamp, 'Session created');
    } else {
      session_config.started_at = heartbeat_data.started_at;
      var heartbeat_response = createHeartbeatResponse(heartbeat_data,
        session_id, new_timestamp, 'Session updated');
    }

    storage.hset(`hb_sessions:${user_id}:active_sessions`, session_id, JSON.stringify(session_config));
    if (!!req.body.progress)
      storage.hset('user_progress', `${user_id}:${heartbeat_data.asset_id}`, req.body.progress);
...
function clearAndGetActiveSessions(sessions, storage, heartbeat_data, new_timestamp) {
  ...
  for (session_id of Object.keys(sessions)) {
    ...
    if (!timeExceeded(session.timestamp,
        heartbeat_data.heartbeat_cycle, heartbeat_data.cycle_upper_tolerance, new_timestamp)) {
